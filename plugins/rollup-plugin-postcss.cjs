  ...
  process({ code, map }) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const postcssOptions = _objectSpread2(
        _objectSpread2(_objectSpread2({}, _this.options.postcss), config.options),
        {},
        {
      ...
      if (options.namedExports) {
        ...
        const getClassName =
          typeof options.namedExports === 'function' ? options.namedExports : ensureClassName;
...
const resolvePromise = pify__default['default'](resolve__default['default']);
...
class Loaders {
  constructor(options = {}) {
    ...
    const customPostcssLoader = _objectSpread2(
      ...
      {
        test: (filepath) =>
          extensions.some((ext) => path__default['default'].extname(filepath) === ext)
      }
    );
      ...
      return _asyncToGenerator(function* () {
        ...
        const getExtracted = () => {
          ...
          if (typeof postcssLoaderOptions.extract === 'string') {
            fileName = path__default['default'].isAbsolute(postcssLoaderOptions.extract)
              ? normalizePath(path__default['default'].relative(dir, postcssLoaderOptions.extract))
          ...
          const _bundle$normalizePath =
              bundle[normalizePath(path__default['default'].relative(dir, file))],
        ...
        let _getExtracted = getExtracted(),
          ...
          mapFileName = _getExtracted.mapFileName;
        ...
        if (postcssLoaderOptions.minimize) {
          ...
          const result = yield require('cssnano')(postcssLoaderOptions.minimize).process(
            code,
            cssOptions
          );
          code = result.css;
