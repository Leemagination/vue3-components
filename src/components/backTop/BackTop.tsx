import {
  computed,
  defineComponent,
  h,
  onMounted,
  onUnmounted,
  PropType,
  ref,
  renderSlot,
  Teleport,
  Transition,
  watch
} from 'vue';
import style from './backTop.scss';

const backTopProps = {
  visibleTop: {
    type: Number,
    default: 100
  },
  bottom: Number,
  right: Number,
  target: {
    type: Object as PropType<HTMLElement>,
    default: undefined
  }
};

const defaultIcon = (
  <svg
    class="lee-back-top-arrow"
    height="100%"
    width="100%"
    viewBox="0 0 24 24"
    version="1.1"
    xmlns="http://www.w3.org/2000/svg"
    xlinkHref="http://www.w3.org/1999/xlink"
  >
    <g stroke="none" stroke-width="1" fill-rule="evenodd">
      <g transform="translate(-139.000000, -4423.000000)" fill-rule="nonzero">
        <g transform="translate(120.000000, 4285.000000)">
          <g transform="translate(7.000000, 126.000000)">
            <g transform="translate(24.000000, 24.000000) scale(1, -1) translate(-24.000000, -24.000000) translate(12.000000, 12.000000)">
              <g transform="translate(4.000000, 2.000000)">
                <path
                  fill="currentColor"
                  d="M8,0 C8.51283584,0 8.93550716,0.38604019 8.99327227,0.883378875 L9,1 L9,10.584 L12.2928932,7.29289322 C12.6834175,6.90236893 13.3165825,6.90236893 13.7071068,7.29289322 C14.0675907,7.65337718 14.0953203,8.22060824 13.7902954,8.61289944 L13.7071068,8.70710678 L8.70710678,13.7071068 L8.62544899,13.7803112 L8.618,13.784 L8.59530661,13.8036654 L8.4840621,13.8753288 L8.37133602,13.9287745 L8.22929083,13.9735893 L8.14346259,13.9897165 L8.03324678,13.9994506 L7.9137692,13.9962979 L7.77070917,13.9735893 L7.6583843,13.9401293 L7.57677845,13.9063266 L7.47929125,13.8540045 L7.4048407,13.8036865 L7.38131006,13.7856883 C7.35030318,13.7612383 7.32077858,13.7349921 7.29289322,13.7071068 L2.29289322,8.70710678 L2.20970461,8.61289944 C1.90467972,8.22060824 1.93240926,7.65337718 2.29289322,7.29289322 C2.65337718,6.93240926 3.22060824,6.90467972 3.61289944,7.20970461 L3.70710678,7.29289322 L7,10.585 L7,1 L7.00672773,0.883378875 C7.06449284,0.38604019 7.48716416,0 8,0 Z"
                ></path>
                <path
                  fill="currentColor"
                  d="M14.9333333,15.9994506 C15.5224371,15.9994506 16,16.4471659 16,16.9994506 C16,17.5122865 15.5882238,17.9349578 15.0577292,17.9927229 L14.9333333,17.9994506 L1.06666667,17.9994506 C0.477562934,17.9994506 0,17.5517354 0,16.9994506 C0,16.4866148 0.411776203,16.0639435 0.9422708,16.0061783 L1.06666667,15.9994506 L14.9333333,15.9994506 Z"
                ></path>
              </g>
            </g>
          </g>
        </g>
      </g>
    </g>
  </svg>
);

const BackTop = defineComponent({
  __STYLE__: style,
  name: 'BackTop',
  props: backTopProps,
  setup(props) {
    const buttonVisible = ref(false);

    const bottomDis = computed(() => {
      return `${props.bottom || 40}px`;
    });
    const rightDis = computed(() => {
      return `${props.right || 40}px`;
    });

    const listenerTarget = computed(() => {
      if (props.target instanceof HTMLElement) {
        return props.target;
      }
      return window;
    });

    watch(listenerTarget, (newTarget, oldTarget) => {
      removeScrollListener(oldTarget);
      addScrollListener();
    });

    const scrollTarget = computed(() => {
      return listenerTarget.value === window ? document.documentElement : listenerTarget.value;
    });

    async function scrollToTop() {
      scrollTarget.value.scrollTo({ top: 0, behavior: 'smooth' });
    }

    function scrollHandle(e: Event) {
      let scrollDom = e.target as HTMLElement;
      if (listenerTarget.value === window) {
        scrollDom = document.documentElement;
      }
      buttonVisible.value = scrollDom.scrollTop > props.visibleTop;
    }
    function addScrollListener() {
      if (props.visibleTop === 0) {
        buttonVisible.value = true;
        return;
      }
      listenerTarget.value.addEventListener('scroll', scrollHandle);
    }

    function removeScrollListener(target?: HTMLElement | Window) {
      if (target) {
        target.removeEventListener('scroll', scrollHandle);
        return;
      }
      listenerTarget.value.removeEventListener('scroll', scrollHandle);
    }

    onMounted(() => {
      addScrollListener();
    });

    onUnmounted(() => {
      removeScrollListener();
    });

    return {
      bottomDis,
      rightDis,
      buttonVisible,
      scrollToTop
    };
  },
  render() {
    const { $slots } = this;
    const slot = renderSlot($slots, 'default');
    return (
      <Teleport to="body">
        <Transition name="lee-back-top-fade">
          {this.buttonVisible ? (
            <div
              onClick={this.scrollToTop}
              class={'lee-back-top'}
              style={{ bottom: this.bottomDis, right: this.rightDis }}
            >
              {$slots.default ? slot : defaultIcon}
            </div>
          ) : null}
        </Transition>
      </Teleport>
    );
  }
});

export default BackTop;
